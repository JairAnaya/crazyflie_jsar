...
class TrajectoryCircle:
    def __init__(self):
        ...
        rospy.Subscriber("external_pose", PoseStamped, self.update_initial_position)

    def update_initial_position(self, pose):
        ...
        while not rospy.is_shutdown():
            ...
            if self.button_pressed:  # Verificar si la trayectoria ha sido iniciada
                ...
                if not self.reached_altitude:
                    z = self.altura * (1 - np.exp(-0.5 * elapsed_time)) + self.zi
                    yaw = 0

                    full_msg.twist1 = Twist()
                    full_msg.twist1.linear.x = self.initial_x
                    full_msg.twist1.linear.y = self.initial_y
                    ...
                    full_msg.twist2.linear.x = 0.0
                    full_msg.twist2.linear.y = 0.0
                    full_msg.twist2.linear.z = 0.0
                    full_msg.twist2.angular.x = yaw
                    full_msg.twist2.angular.y = 0.0
                    full_msg.twist2.angular.z = 0.0

                    if abs(z - self.altura) < 0.005:
                        rospy.loginfo("Altura alcanzada")
                        self.reached_altitude = True
                        self.start_time = rospy.Time.now()

                elif self.reached_altitude:
                    elapsed_time = (current_time - self.start_time).to_sec()
                    self.t = elapsed_time
                    w = np.pi / 6
                    p = 15
